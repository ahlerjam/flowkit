...
const WAIT_SCHEMA = {
  ...
  properties: {
    ...
    retriggered: { type: 'boolean', description: `true NUR, wenn die Station den einen erlaubten Re-Trigger tatsächlich ausgeführt hat: ein BEHIND-Update (git merge origin/${BRANCH} im eigenen Worktree, gepusht) — ein Draft-Toggle und ein leerer Commit sind als Re-Trigger nachweislich wirkungslos und deshalb verboten. Ein Update, das ein ANDERER Akteur gefahren hat (pr-autoupdate-Workflow, Mensch), zählt hier nicht mit: false, Vorgang in die note` },
...
// Zweiter Akteur auf demselben Branch (0.10.0): seit es das CI-Template
// pr-autoupdate.yml gibt, zieht ein Repo seine offenen PR-Branches nach JEDEM
// Push auf den Default-Branch selbsttätig nach — genau das, was diese Station
// reaktiv tut. Beide serialisieren sich NICHT gegeneinander, und das ist
// Absicht: ein Lock über Prozessgrenzen hinweg gäbe es nur als Label am PR,
// und ein Label, das ein abgestürzter Lauf stehen lässt, nähme den PR dauerhaft
// aus der Automatik — ein stiller Dauerschaden gegen ein Sekundenfenster.
// Stattdessen konvergieren beide: es ist dieselbe idempotente Operation (merge
// origin/<default>, kein Rebase, kein Force), also ist ein abgelehnter Push
// keine Störung, sondern die Auskunft, dass der andere schneller war —
// Schritt 3c liest den Zustand neu und gibt nach oder wiederholt genau einmal.
// Ohne diese Regel hätte ein verlorenes Rennen die Einheit auf needs-human
// geschickt, obwohl der Branch danach genau richtig stand. Aus demselben Grund
// prüft Schritt 3a beim "schon enthalten"-Ausgang den HEAD-SHA gegen Schritt 1:
// ein fremdes Update sieht von hier aus zunächst aus wie ein No-op-Trigger.
// Was NICHT abgedeckt ist und deshalb im Template dokumentiert steht: landet
// ein fremdes Update in den Sekunden zwischen dem Grün dieser Station und dem
// gh pr merge der Merge-Station, verweigert die Branch-Protection den Merge
// (Checks am neuen HEAD laufen noch) und die Einheit endet blockiert. Ein
// hängengebliebener PR, nie ein ungeprüfter Merge.
...
3a. DER RE-TRIGGER IST EIN BEHIND-UPDATE, nichts anderes. Live gemessen (2026-08-01, PR #576/#580): gh pr ready bzw. ein Draft-Toggle löste KEINEN Lauf aus, ein leerer Commit (git commit --allow-empty) plus Push ebenfalls KEINEN — erst ein echtes Update vom ${BRANCH} ließ die Läufe binnen Sekunden anlaufen. Also: KEIN Draft-Toggle (du setzt den PR in dieser Station unter keinen Umständen auf Draft zurück — er bleibt in JEDEM Ausgang ready), KEIN leerer Commit, KEIN gh run rerun (es gibt keinen Lauf, den man wiederholen könnte — Schritt 4a greift nur bei einem ROTEN Lauf). Ablauf: git fetch origin, dann prüfen, ob der Branch ${BRANCH} überhaupt noch nicht enthält — git merge-base --is-ancestor origin/${BRANCH} origin/${branch}; Exit 0 heißt "schon enthalten", der Merge wäre ein No-op, pushte nichts und löste nichts aus — dann NICHT triggern. Bevor du daraus einen Endbefund machst, klär WARUM: gh pr view ${pr} -R ${SLUG} --json headRefOid. Weicht der SHA von dem aus Schritt 1 ab, hat ein anderer Akteur den Branch inzwischen vom ${BRANCH} nachgezogen (der pr-autoupdate-Workflow des Repos oder ein Mensch) — das Update ist also bereits gelaufen und mit ihm die Läufe: runsFound ab hier gegen den NEUEN SHA zählen, weiterwarten wie in Schritt 2, retriggered: false (du selbst hast nicht getriggert) und den Grund in die note. NUR bei unverändertem SHA ist der Fall zu Ende: retriggered: false, Grund in die note, beenden wie in Schritt 6. Sonst eigenen Worktree anlegen (git worktree add <tmp> ${branch}, NIE den Haupt-Tree), git merge origin/${BRANCH} (KEIN rebase, KEIN force, KEIN --allow-empty), via ${PUSH} pushen (lehnt der Push ab: Schritt 3c), git worktree remove, retriggered: true melden und weiterwarten — der PR hat jetzt einen neuen HEAD-SHA, hol ihn dir mit gh pr view ${pr} -R ${SLUG} --json headRefOid neu und zähl runsFound ab hier gegen den NEUEN Wert. Kommt auch danach kein Lauf, liegt die Ursache außerhalb dieses PRs: KEIN zweiter Re-Trigger — beenden wie in Schritt 6.
...
3c. ABGELEHNTER PUSH beim BEHIND-Update (${PUSH} endet non-zero mit non-fast-forward, "fetch first" oder "behind its remote counterpart"): das ist KEIN Fehler und unter keinen Umständen ein Grund für --force oder --force-with-lease. Es heißt nur, dass ein anderer Akteur denselben Branch in derselben Sekunde vom ${BRANCH} nachgezogen hat — der pr-autoupdate-Workflow des Repos tut nach jedem Merge genau das, ein Mensch von Hand ebenso, und es ist DIESELBE Operation wie deine. Ablauf: git fetch origin, dann git merge-base --is-ancestor origin/${BRANCH} origin/${branch}. Exit 0 → die Arbeit ist erledigt, du gibst nach: Worktree entfernen, NICHT erneut pushen, retriggered: false (getriggert hat der andere), neuen headRefOid holen, runsFound gegen ihn zählen, weiterwarten wie in Schritt 2 und den Vorgang in die note. Sonst GENAU EINEN Wiederholungsversuch (Worktree verwerfen, frisch auf ${branch} neu anlegen, erneut mergen und pushen); scheitert auch der, Worktree entfernen, retriggered: false, Grund in die note und beenden wie in Schritt 6 — zwei verlorene Rennen hintereinander sind kein Zufall mehr, sondern ein Befund für den Operator.
